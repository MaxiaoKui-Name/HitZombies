...
public class XLog
{
    ...
    // 警告日志开关
    public static bool s_warningLogEnable = false;
    // 错误日志开关
    ...
    {
        // 日期
        var t = System.DateTime.Now.ToString("yyyyMMdd_HH_mm_ss");
        ...
        //删除旧日志
        DeleteOldLogs(logDir, 3);

    }

    //删除旧日志 保留oldDay天日志
    private static void DeleteOldLogs(string logDir, int oldDay = 30)
    {
        try
        {
            // 获取所有日志文件
            ...
            foreach (var logFile in logFiles)
            {
                ...
                // 如果文件创建时间早于oldDay天前，删除该文件
                if (creationTime < System.DateTime.Now.AddDays(-oldDay))
                {
                    File.Delete(logFile);
                }
            }
        ...
        {
            // 处理异常（例如权限问题等）
            LogError($"Error deleting old logs: {e.Message}");
        }
    }
